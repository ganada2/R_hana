...
# 작업폴더 설정 - setwd()
setwd("C:/Users/hanauser10/Documents/R_hana")
...
rep(colnames(aws)[6],"aa")
head(aws)

colnames(aws)[6] = "A6"


colnames(aws)[c(6,7)] = c("N6", "N7")
head(aws)
...
rowper = paste0(round(prop.table(table(diamonds$cut))*100, 2), "%")
rowper
...
dataf = data.frame(aa = unique(diamonds$cut), bb = rowper)
dataf

fef = names(table(diamonds$cut))
fef
...
quantile(df$Wind, probs = c(0.1, 0.9, 0.5))

quantile(df$Wind, probs = 1:10 * 0.1 )
...
paste(sample[,1], sample[,2], sep= " HAHAHAA")
...
paste0(c("123","123","123"))

paste0(123,123,123,sep = "-")
paste(c(123,123,123),sep = "-")
...
# 응용
for ( n in 1:4){ df_1[n,"new_COL"] = n * 4
print(df_1)}


#### ____ ● 조건문 - if() ####
# if() 함수
if(n == 3){ print("n 3")}else{ print("not 3")}
